...
def findContoursInFile(filename):
    ...
    image_bw = cv2.threshold(image_grey, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
...
def main():
    ...
    for idx, values in enumerate(output):
        ...
        if area > 400:
            ...
            column = int(coordy * 9 / 800)
            row = int(coordx * 9 / 800)
            grid[column][row] = recognizedClass

    print()
    if (sudoku_solver.Suduko(grid, 0, 0)):
        sudoku_solver.puzzle(grid)
    else:
    ...
    for idx, values in enumerate(output):
        for column in range(9):
            for row in range(9):
                coordy = int((row + 0.5) * 800 / 9)
                coordx = int((column + 0.5) * 800 / 9)
                cv2.putText(drawing, "%d" % grid[row][column],
                        (coordx, coordy), 
                        cv2.FONT_HERSHEY_TRIPLEX,
                        1, 
                        (0, 0, 255))
